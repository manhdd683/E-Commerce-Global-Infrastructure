...
import { 
  ...
  FaClipboardList, FaBoxOpen, FaTruck, FaGift, FaWallet, FaStar, FaTimesCircle, FaGlobe, FaHandshake 
} from 'react-icons/fa';
...
const TRANSLATIONS = {
  vi: {
    ...
    logout: "Đăng xuất",
    login: "Đăng nhập",
    featureUpdating: "Chức năng đang được cập nhật!"
  },
  en: {
    ...
    logout: "Log out",
    login: "Login",
    featureUpdating: "Feature is updating!"
  },
  ja: {
...
const Header = () => {
    ...
    const fetchOrderNotifications = async () => {
      try {
        ...
        myOrders.forEach(order => {
          ...
          if (message) {
            generatedNotifs.push({ id: `order_${order.id}`, title: 'Cập nhật Đơn hàng', message, timestamp: new Date(order.orderDate).getTime(), iconType, link: '/my-orders' });
          }
        });
  ...
  return (
    <header style={{ backgroundColor: 'white', borderBottom: '1px solid #eaeaea', position: 'sticky', top: 0, zIndex: 1000, boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px 20px' }}>
        
        <Link to="/" style={{ textDecoration: 'none' }}>
          <h1 style={{ margin: 0, color: '#ff469e', fontSize: '28px', fontWeight: '900' }}>E-Commerce</h1>
        </Link>

        <form onSubmit={handleSearch} style={{ display: 'flex', width: '40%', border: '2px solid #ff469e', borderRadius: '4px', overflow: 'hidden' }}>
          <input type="text" placeholder={t.searchPlaceholder} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ flex: 1, padding: '10px 15px', border: 'none', outline: 'none', fontSize: '14px' }} />
          <button type="submit" style={{ width: '60px', backgroundColor: '#ff469e', color: 'white', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center' }}><FaSearch size={16} /></button>
        </form>

        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          
          <div ref={langRef} style={{ position: 'relative' }}>
            <div onClick={() => setIsLangOpen(!isLangOpen)} style={{ display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer', color: '#555', fontSize: '14px', fontWeight: 'bold' }}>
              <FaGlobe size={18} /> {langConfig[language].flag}
            </div>
            {isLangOpen && (
          ...
          <div ref={notifRef} style={{ position: 'relative' }}>
            ...
            {isNotifOpen && (
              <div style={{ position: 'absolute', top: '35px', right: '-80px', width: '380px', backgroundColor: 'white', borderRadius: '8px', boxShadow: '0 5px 20px rgba(0,0,0,0.15)', border: '1px solid #eee', overflow: 'hidden' }}>
          ...
          {user ? (
            <div ref={dropdownRef} style={{ position: 'relative' }}>
              <div onClick={() => setIsDropdownOpen(!isDropdownOpen)} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', backgroundColor: '#f8f9fa', padding: '6px 15px', borderRadius: '20px', border: '1px solid #eee' }}>
                <img src={user.avatar || 'https://via.placeholder.com/30'} alt="User" style={{ width: '28px', height: '28px', borderRadius: '50%', objectFit: 'cover' }} />
                <span style={{ fontWeight: '600', color: '#333', fontSize: '14px' }}>{user.username || user.name}</span>
              </div>
              
              {isDropdownOpen && (
                <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '10px', width: '250px', backgroundColor: 'white', borderRadius: '8px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', border: '1px solid #eee', overflow: 'hidden' }}>
                  ...
                  {/* ĐÃ FIX: CHUYỂN HƯỚNG SANG TRANG AFFILIATE, KHÔNG ALERT BÁO LỖI NỮA */}
            ...
            <Link to="/login" style={{ padding: '8px 20px', border: '1px solid #ddd', borderRadius: '4px', textDecoration: 'none', color: '#333', fontWeight: 'bold' }}>{t.login}</Link>
          )}
