  ...
  return (
    <div className="d-flex flex-column flex-md-row min-vh-100 bg-light">
      ...
      <style>{`
        ...
        .modern-table th { background-color: #f8f9fa !important; border-bottom: 2px solid #eaeaea !important; }
        .modern-table td { border-bottom: 1px solid #f2f2f2; vertical-align: middle; }
      ...
      {/* ================= NỘI DUNG CHÍNH ================= */}
      <div className="flex-grow-1 p-3 p-md-4 p-lg-5 overflow-auto" style={{ width: '100%', backgroundColor: '#f4f6f9' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          {isLoading ? (
            ...
            <>
                ...
                <div>
                  ...
                  <div className="row g-4">
                    <div className="col-12 col-lg-5">
                      <div className="bg-white p-5 rounded-4 shadow-sm border-0 h-100">
              ...
              {/* TAB 1: BÁO CÁO DOANH THU (Đã sửa lỗi hiển thị màu che chữ) */}
              {activeTab === "overview" && (
                <div>
                  <h1 className="fs-3 text-dark mb-4 fw-black">Báo cáo tài chính</h1>
                  <div className="row g-4 mb-4">
                    <div className="col-12 col-md-4">
                      <div className="bg-white p-4 rounded-4 shadow-sm border-0 border-start border-success border-5 h-100 d-flex flex-column justify-content-center">
                        <div className="text-success small fw-bold mb-2 text-uppercase" style={{ letterSpacing: '1px' }}>Tổng GMV (Tiền hàng)</div>
                        <div className="fs-2 text-dark fw-black">{totalGMV.toLocaleString("vi-VN")} ₫</div>
                      </div>
                    </div>
                    <div className="col-12 col-md-4">
                      <div className="bg-white p-4 rounded-4 shadow-sm border-0 border-start border-primary border-5 h-100 d-flex flex-column justify-content-center">
                        <div className="text-primary small fw-bold mb-2 text-uppercase" style={{ letterSpacing: '1px' }}>Lợi nhuận Sàn</div>
                        <div className="fs-2 fw-black" style={{ color: '#0d6efd' }}>{totalPlatformRevenue.toLocaleString("vi-VN")} ₫</div>
                        <div className="small text-muted mt-1">Gồm hoa hồng & Dịch vụ</div>
                      </div>
                    </div>
                    <div className="col-12 col-md-4">
                      <div className="bg-white p-4 rounded-4 shadow-sm border-0 border-start border-warning border-5 h-100 d-flex flex-column justify-content-center">
                        <div className="text-warning small fw-bold mb-2 text-uppercase" style={{ letterSpacing: '1px' }}>Tổng đơn hoàn thành</div>
                        <div className="fs-2 text-dark fw-black">{productOrders.length} <span className="fs-5 text-muted fw-normal">đơn</span></div>
                      </div>
                    </div>
                  </div>
              ...
              {activeTab === "approvals" && (
                <div>
                  ...
                  {pendingServiceRequests.length === 0 ? (
                    ...
                    <div className="bg-white rounded-4 p-4 shadow-sm border-0 border-top border-warning border-5">
                      <h2 className="fs-5 text-warning fw-bold d-flex align-items-center gap-2 mb-3">
                        <FaExclamationTriangle /> Cần xử lý: {pendingServiceRequests.length} yêu cầu
                      </h2>
                      <p className="text-secondary small mb-4">Vui lòng kiểm tra sao kê ngân hàng trước khi bấm "Đã nhận tiền" để kích hoạt gói cho Seller.</p>
                      
                      <div className="table-responsive">
                        <table className="table modern-table mb-0">
                          <thead className="text-uppercase small text-secondary fw-bold" style={{ letterSpacing: '0.5px' }}>
                            <tr>
                              <th className="py-3">Seller Yêu cầu</th>
                              <th className="py-3">Nội dung gói</th>
                              <th className="py-3 text-nowrap">Số tiền chuyển</th>
              ...
              {/* TAB 3: QUẢN LÝ TÀI KHOẢN (Đã hiển thị rõ mật khẩu) */}
              {activeTab === "users" && (
                <div>
                  {showAddModal && (
                  ...
                  <div className="bg-white rounded-4 shadow-sm border-0 overflow-hidden p-3">
                    <div className="table-responsive">
                      <table className="table modern-table align-middle mb-0">
                        <thead className="text-uppercase small text-secondary fw-bold" style={{ letterSpacing: '0.5px' }}>
                          <tr>
                            <th className="py-3 px-4">Tên người dùng</th>
                            <th className="py-3">Mật khẩu</th>
                          ...
                          {users.length === 0 ? (
                            ...
                            users.map((u) => (
                              <tr key={u.id} className={u.status === "Suspended" ? "opacity-50" : ""}>
                                <td className="fw-bold text-dark py-3 px-4">{u.username}</td>
                                <td className="py-3 text-primary font-monospace fw-bold">{u.password || "N/A"}</td>
                                <td className="py-3">
                                  <select
                                    value={u.role ? u.role.toUpperCase() : "USER"}
                                    onChange={(e) => handleChangeRole(u.id, e.target.value)}
                                    disabled={u.status === "Suspended"}
                                    className="form-select form-select-sm fw-bold bg-light border-0 shadow-none w-auto cursor-pointer"
                                  >
                                ...
                                <td className="py-3 text-end px-4 text-nowrap">
              ...
              {activeTab === "products" && (
                <div>
                  ...
                  <div className="bg-white rounded-4 shadow-sm border-0 overflow-hidden p-3">
                      ...
                      <table className="table modern-table align-middle mb-0">
                        <thead className="text-uppercase small text-secondary fw-bold" style={{ letterSpacing: '0.5px' }}>
                          <tr>
                            ...
                            <th className="py-3 text-center" style={{ width: "80px" }}>Ảnh</th>
                            <th className="py-3" style={{ minWidth: "250px" }}>Tên sản phẩm</th>
                            <th className="py-3 text-center">Tình trạng</th>
                            <th className="py-3 text-end px-4 text-nowrap">Thao tác xử lý</th>
                          </tr>
                        </thead>
                        <tbody>
                          {currentProducts.length === 0 ? (
                            ...
                            currentProducts.map((item, index) => (
                              <tr key={item.id} className={item.moderationStatus === "Banned" ? "bg-danger bg-opacity-10" : ""}>
                                <td className="text-center fw-bold text-muted py-3 px-4">{indexOfFirstProduct + index + 1}</td>
                    ...
                    {totalProductPages > 1 && (
                      <div className="d-flex justify-content-center gap-2 pt-4 pb-2 bg-white">
                        <button disabled={currentProductPage === 1} onClick={() => setCurrentProductPage((prev) => prev - 1)} className="btn btn-light rounded-circle" style={{ width: '40px', height: '40px' }}>&lt;</button>
                        ...
                        <button disabled={currentProductPage === totalProductPages} onClick={() => setCurrentProductPage((prev) => prev + 1)} className="btn btn-light rounded-circle" style={{ width: '40px', height: '40px' }}>&gt;</button>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="bg-white rounded-4 shadow-sm border-0 overflow-hidden p-3">
                    <div className="table-responsive">
                      <table className="table modern-table align-middle mb-0">
                        <thead className="text-uppercase small text-secondary fw-bold" style={{ letterSpacing: '0.5px' }}>
                          <tr>
                            <th className="py-3 px-4 text-nowrap">Mã Lệnh</th>
                            <th className="py-3 text-nowrap">Người mua</th>
                            <th className="py-3 text-nowrap">Tổng giá trị</th>
                            <th className="py-3 text-nowrap">Tiến độ</th>
                            <th className="py-3 text-center text-nowrap">Hệ thống phân tích</th>
                          </tr>
                        </thead>
                        <tbody>
                            ...
                            currentOrders.map((order) => {
                              ...
                              return (
                                <tr key={order.id} className={order.isSuspicious || isHighValue ? "bg-warning bg-opacity-10" : ""}>
                                  <td className="fw-black text-primary py-3 px-4">#{order.id}</td>
                                  <td className="fw-bold text-dark py-3">{order.customerInfo?.fullName || order.username || "Khách ẩn danh"}</td>
                                  <td className="fw-bold text-danger py-3 text-nowrap fs-6">{Number(order.totalPrice || 0).toLocaleString("vi-VN")} ₫</td>
                                  <td className="py-3">
                    ...
                    {totalPages > 1 && (
                      <div className="d-flex justify-content-center gap-2 pt-4 pb-2 bg-white">
                        <button disabled={currentPage === 1} onClick={() => setCurrentPage((prev) => prev - 1)} className="btn btn-light rounded-circle" style={{ width: '40px', height: '40px' }}>&lt;</button>
                        ...
                        <button disabled={currentPage === totalPages} onClick={() => setCurrentPage((prev) => prev + 1)} className="btn btn-light rounded-circle" style={{ width: '40px', height: '40px' }}>&gt;</button>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="bg-white rounded-4 shadow-sm border-0 overflow-hidden p-3">
                    <div className="table-responsive">
                      <table className="table modern-table align-middle mb-0">
                        <thead className="text-uppercase small text-secondary fw-bold" style={{ letterSpacing: '0.5px' }}>
                          <tr>
                            <th className="py-3 px-4 text-nowrap">Danh tính Code</th>
                            <th className="py-3 text-nowrap">Giá trị thả</th>
                            <th className="py-3 text-nowrap">Điều kiện đơn</th>
                            <th className="py-3 text-center text-nowrap">Hạn ngạch</th>
                            <th className="py-3 text-center text-nowrap">Đếm ngược hạn</th>
                            <th className="py-3 text-end px-4 text-nowrap">Can thiệp</th>
                          </tr>
                        </thead>
                        <tbody>
                          {vouchers.length === 0 ? (
                            ...
                            vouchers.map((v, index) => {
                              ...
                              return (
                                <tr key={index} className={isExpired ? "opacity-50" : ""}>
                                  ...
                                  <td className="fw-bold text-dark py-3 text-nowrap fs-6">{v.type === 'PERCENT' ? `${v.value}%` : `${v.value.toLocaleString('vi-VN')} ₫`}</td>
                                  <td className="text-dark py-3 text-nowrap fw-bold">{v.minSpend.toLocaleString('vi-VN')} ₫</td>
                                  <td className="text-dark py-3 text-center text-nowrap fw-bold">
