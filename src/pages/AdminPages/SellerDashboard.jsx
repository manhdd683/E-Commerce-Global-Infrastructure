...
import { 
  ...
  FaClipboardList, FaCheckCircle, FaChartBar, FaBullhorn, FaMoneyBillWave, FaComments, FaExclamationCircle, FaWallet, FaFileInvoiceDollar, FaBell, FaStar, FaPaperPlane, FaVideo, FaCog, FaFileExcel, FaTicketAlt, FaDownload
} from 'react-icons/fa';
...
const SellerDashboard = () => {
  ...
  const loadSystemPackages = () => {
    ...
    setMarketingPackages(savedPackages);
  };
  ...
  useEffect(() => {
    if (currentSellerName) {
      fetchProducts(); fetchOrders(); loadSellerChats(); loadShopVouchers(); loadSystemPackages();
  ...
  const handleExportExcel = () => {
    const completedOrders = regularOrders.filter(o => o.status === 'Completed');
    if (completedOrders.length === 0) {
      return alert("Chưa có đơn hàng hoàn tất nào để xuất báo cáo!");
    }

    const headers = ["Mã Đơn Hàng", "Ngày Đặt", "Khách Hàng", "Sản Phẩm", "Tổng Tiền (VNĐ)", "Trạng Thái"];
    ...
    link.setAttribute("download", `BaoCaoDoanhThu_${currentSellerName}_${new Date().getTime()}.csv`);
  ...
  const getThemeForPackage = (type) => {
    switch(type) {
      case 'FREESHIP': return 'danger';
      case 'ADS': return 'primary';
      case 'FLASH_SALE': return 'warning';
  ...
  return (
    <div className="d-flex flex-column flex-md-row vh-100 w-100 bg-light" style={{ overflowX: 'hidden' }}>
      ...
      <style>{`
        #seller-sidebar { width: 100%; overflow-x: auto; border-bottom: 1px solid #e9ecef; }
        @media (min-width: 768px) {
          #seller-sidebar { width: 260px; min-width: 260px; height: 100vh; position: sticky; top: 0; overflow-y: auto; border-bottom: none; }
        }
        ...
        /* Đập đi xây lại CSS Table chuẩn xịn */
        .modern-table th { 
      ...
      <div id="seller-sidebar" className="bg-white shadow-sm d-flex flex-md-column flex-shrink-0">
        <div className="p-4 border-bottom text-center">
          <h2 className="fs-5 text-danger fw-black mb-1 text-uppercase">Kênh Người Bán</h2>
          <div className="small text-muted fw-bold">{displayShopName}</div>
        </div>
      ...
      <div className="dashboard-content flex-grow-1">
        <div style={{ maxWidth: '1300px', margin: '0 auto' }}>
          ...
          {activeTab === 'analytics' && (
            <div>
              <h1 className="fs-3 fw-black text-dark mb-4">Hiệu quả hoạt động</h1>
              
              <div className="row g-4 mb-4">
                <div className="col-12 col-md-4">
                  <div className="content-card h-100 d-flex align-items-center gap-4">
                    <div className="bg-danger bg-opacity-10 text-danger p-3 rounded-circle d-flex justify-content-center align-items-center flex-shrink-0" style={{width: '64px', height: '64px'}}>
                      <FaMoneyBillWave size={28} />
                    </div>
                    <div>
                      <div className="text-secondary small fw-bold mb-1 text-uppercase" style={{ letterSpacing: '1px' }}>Doanh thu thực tế</div>
                      <div className="fs-3 text-dark fw-black">{totalRevenue.toLocaleString('vi-VN')} ₫</div>
                    </div>
                  </div>
                </div>
                <div className="col-12 col-md-4">
                  <div className="content-card h-100 d-flex align-items-center gap-4">
                    ...
                    <div className="flex-grow-1">
                      <div className="text-secondary small fw-bold mb-1 text-uppercase" style={{ letterSpacing: '1px' }}>Chờ xử lý</div>
                      <div className="fs-3 text-dark fw-black mb-1">{pendingOrdersCount} <span className="fs-6 text-muted fw-normal">đơn</span></div>
                      <div className="small text-primary fw-bold cursor-pointer" onClick={() => setActiveTab('orders')}>Xử lý ngay &rarr;</div>
                    </div>
                  </div>
                </div>
                <div className="col-12 col-md-4">
                  <div className="content-card h-100 d-flex align-items-center gap-4">
                    ...
                    <div>
                      <div className="text-secondary small fw-bold mb-1 text-uppercase" style={{ letterSpacing: '1px' }}>Cảnh báo tồn kho</div>
                      <div className="fs-3 text-dark fw-black">{lowStockProducts.length} <span className="fs-6 text-muted fw-normal">SP</span></div>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div style={{ height: '350px', width: '100%' }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{ top: 20, right: 10, left: 20, bottom: 5 }}>
                      ...
                      <YAxis allowDecimals={false} tickFormatter={formatYAxis} width={85} axisLine={false} tickLine={false} tick={{fontSize: 12, fill: '#888'}} />
                      ...
                      <Bar dataKey="revenue" fill="#ee4d2d" radius={[8, 8, 0, 0]} maxBarSize={50} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          {activeTab === 'products' && (
            <div>
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <h1 className="fs-3 text-dark m-0 fw-black">Quản lý Sản phẩm</h1>
                <div className="d-flex gap-2">
                  <button onClick={() => navigate('/seller/bulk-upload')} className="btn btn-outline-success fw-bold d-flex align-items-center gap-2 px-4 rounded-pill">
                    <FaFileExcel /> <span className="d-none d-sm-inline">Nhập Excel</span>
                  </button>
                  <button onClick={() => setIsModalOpen(true)} className="btn btn-danger fw-bold d-flex align-items-center gap-2 px-4 rounded-pill shadow-sm">
                ...
                {isLoadingProducts ? (
                  ...
                  <>
                    <div className="table-responsive">
                      <table className="table modern-table w-100 mb-0">
                        ...
                        <tbody>
                          {currentProducts.length === 0 ? (
                            <tr>
                              <td colSpan="5" className="text-center text-muted py-5">Chưa có sản phẩm nào thuộc shop này</td>
                            </tr>
                          ) : (
                            currentProducts.map((item) => (
                              <tr key={item.id}>
                                <td className="text-center">
                                  <div className="bg-light border rounded-3 d-inline-flex justify-content-center align-items-center" style={{ width: '50px', height: '50px' }}>
                                    <img src={item.images?.[0] || item.image || "https://via.placeholder.com/50"} alt={item.name} className="object-fit-contain w-100 h-100 p-1" />
                                  </div>
                                </td>
                                ...
                                <td className="text-nowrap">
                                  {item.stock <= 5 ? <span className="text-danger fw-bold">{item.stock} (Sắp hết)</span> : <span className="text-success fw-bold">{item.stock}</span>}
                                </td>
                    ...
                    {totalProductPages > 1 && (
                      <div className="d-flex justify-content-center gap-2 p-4 bg-white border-top">
                        <button onClick={() => setCurrentProductPage(prev => Math.max(prev - 1, 1))} disabled={currentProductPage === 1} className="btn btn-light rounded-circle fw-bold" style={{ width: '40px', height: '40px' }}>&lt;</button>
                        ...
                        <button onClick={() => setCurrentProductPage(prev => Math.min(prev + 1, totalProductPages))} disabled={currentProductPage === totalProductPages} className="btn btn-light rounded-circle fw-bold" style={{ width: '40px', height: '40px' }}>&gt;</button>
                      </div>
                    )}
                  </>
                )}
          ...
          {activeTab === 'orders' && (
            <div>
              <div className="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 gap-3">
                <h1 className="fs-3 text-dark m-0 fw-black">Xử lý Đơn hàng</h1>
                <button onClick={fetchOrders} className="btn btn-light border text-secondary fw-bold rounded-pill px-4">🔄 Làm mới</button>
              </div>
              ...
              <div className="content-card p-0 overflow-hidden">
                  ...
                  <table className="table modern-table w-100 mb-0">
                    ...
                    <tbody>
                      {currentOrders.length === 0 ? (
                        <tr><td colSpan="5" className="text-center text-muted py-5">Chưa có đơn hàng nào</td></tr>
                      ) : (
                        currentOrders.map((order) => (
                          <tr key={order.id} className={order.status === 'Processing' ? 'bg-warning bg-opacity-10' : ''}>
                            <td className={`fw-bold ${order.status === 'Processing' ? 'text-danger' : 'text-dark'}`}>#{order.id}</td>
                            <td>
                              <div className="fw-bold text-dark text-nowrap mb-1">{order.customerInfo?.fullName || order.username || 'Khách ẩn danh'}</div>
                              <div className="text-muted" style={{ fontSize: '13px' }}>SĐT: {order.customerInfo?.phone || 'N/A'}</div>
                            </td>
                            <td>
                              {order.items?.map((item, idx) => (
                                <div key={idx} className="d-flex align-items-center gap-3 mb-2">
                                  <div className="bg-white border rounded-3 p-1 flex-shrink-0 d-flex justify-content-center align-items-center" style={{ width: '45px', height: '45px' }}>
                                    <img src={item.images?.[0] || item.image || "https://via.placeholder.com/45"} alt={item.name} className="object-fit-contain w-100 h-100" />
                                  </div>
                                  <div className="d-flex flex-column">
                                    <span className="fw-bold text-dark text-truncate" style={{ maxWidth: '200px', fontSize: '13px' }}>{item.name}</span>
                                    <span className="text-danger fw-bold" style={{ fontSize: '13px' }}>x{item.quantity}</span>
                                  </div>
                                </div>
                              ))}
                            </td>
                            <td className="text-danger fw-bold text-nowrap">{Number(order.totalPrice || 0).toLocaleString('vi-VN')} ₫</td>
                            <td className="text-center">
                              <select 
                                value={order.status} 
                                onChange={(e) => handleUpdateOrderStatus(order.id, e.target.value)} 
                                className="form-select form-select-sm fw-bold cursor-pointer shadow-none"
                                style={{ 
                                  backgroundColor: (statusOptions.find(o => o.value === order.status)?.color || '#333') + '15', 
                                  color: statusOptions.find(o => o.value === order.status)?.color || '#333',
                                  borderColor: (statusOptions.find(o => o.value === order.status)?.color || '#333') + '40',
                                  borderRadius: '6px',
                                  padding: '6px 12px',
                                  minWidth: '160px',
                                  fontSize: '13px',
                ...
                {totalOrderPages > 1 && (
                  <div className="d-flex justify-content-center gap-2 p-4 bg-white border-top">
                    <button onClick={() => setCurrentOrderPage(prev => Math.max(prev - 1, 1))} disabled={currentOrderPage === 1} className="btn btn-light rounded-circle fw-bold" style={{ width: '40px', height: '40px' }}>&lt;</button>
                    {[...Array(totalOrderPages)].map((_, i) => (
                      <button key={i} onClick={() => setCurrentOrderPage(i + 1)} className={`btn fw-bold rounded-circle shadow-sm ${currentOrderPage === i + 1 ? 'btn-danger' : 'btn-light'}`} style={{ width: '40px', height: '40px' }}>{i + 1}</button>
                    ))}
                    <button onClick={() => setCurrentOrderPage(prev => Math.min(prev + 1, totalOrderPages))} disabled={currentOrderPage === totalOrderPages} className="btn btn-light rounded-circle fw-bold" style={{ width: '40px', height: '40px' }}>&gt;</button>
                  </div>
                )}
              </div>
            </div>
          ...
          {activeTab === "vouchers" && (
            <div>
              <div className="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 gap-3">
                <h1 className="fs-3 text-dark m-0 fw-black">Khuyến Mãi Cửa Hàng</h1>
                  ...
                  <table className="table modern-table w-100 mb-0">
                    ...
                    <tbody>
                      {shopVouchers.length === 0 ? (
                        <tr><td colSpan="6" className="text-center text-muted py-5">Shop chưa tạo mã khuyến mãi nào.</td></tr>
                      ) : (
                          ...
                          return (
                            <tr key={index} className={isExpired ? 'opacity-50' : ''}>
                              <td>
                                ...
                                <div className="text-muted" style={{fontSize: '13px'}}>{v.name}</div>
                              </td>
                              <td className="fw-bold text-dark text-nowrap">{v.type === 'PERCENT' ? `${v.value}%` : `${v.value.toLocaleString('vi-VN')} ₫`}</td>
                              <td className="text-dark text-nowrap">{v.minSpend.toLocaleString('vi-VN')} ₫</td>
                              ...
                              <td className="text-center">
                                <button onClick={() => handleDeleteShopVoucher(v.code)} className="btn btn-outline-danger btn-sm fw-bold rounded-pill px-3 d-inline-flex align-items-center gap-1 shadow-sm">
                                  <FaTrash /> Thu hồi
                                </button>
                              </td>
                            </tr>
          ...
          {activeTab === 'reviews' && (
            <div>
              <h1 className="fs-3 text-dark mb-4 fw-black">Đánh giá sản phẩm</h1>
                        ...
                        <div className="mt-3">
                          {replyingTo === review.id ? (
                            <div className="d-flex gap-2 flex-wrap">
                              <input type="text" value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="Nhập câu trả lời..." className="form-control bg-light border-0 mb-2" />
                              <div className="d-flex gap-2 w-100">
                                 <button onClick={() => handleReplyReview(review.id)} className="btn btn-danger fw-bold px-3 flex-grow-1 rounded-pill">Gửi</button>
                                 <button onClick={() => {setReplyingTo(null); setReplyText('');}} className="btn btn-light border fw-bold flex-grow-1 rounded-pill">Hủy</button>
                              </div>
                            </div>
                          ) : (
                            <button onClick={() => setReplyingTo(review.id)} className="btn btn-outline-danger btn-sm fw-bold px-4 rounded-pill shadow-sm">Phản hồi đánh giá</button>
                          )}
                        </div>
                      )}
                    </div>
          ...
          {activeTab === 'customers' && (
            <div className="content-card p-0 overflow-hidden d-flex flex-column flex-md-row" style={{ height: '75vh' }}>
              <div className="border-end d-flex flex-column bg-white" style={{ width: '100%', maxWidth: '350px' }}>
                <div className="bg-light p-4 border-bottom fw-black fs-5 text-dark">Tin nhắn KH ({chats.length})</div>
                <div className="overflow-auto flex-grow-1">
                  {chats.map(chat => {
                    const unread = chat.messages.filter(m => m.sender === 'user' && !m.isRead).length;
                    return (
                      <div key={chat.id} onClick={() => { setActiveChatId(chat.id); markAsRead(chat.id); }} className={`p-3 border-bottom cursor-pointer d-flex align-items-center gap-3 transition-all ${activeChatId === chat.id ? 'bg-danger bg-opacity-10' : 'bg-white hover-bg-light'}`}>
                        <img src={chat.avatar || 'https://via.placeholder.com/40'} alt="avatar" className="rounded-circle flex-shrink-0 border shadow-sm" style={{ width: '48px', height: '48px', objectFit: 'cover' }} />
                        <div className="overflow-hidden w-100">
                          <div className="d-flex justify-content-between align-items-center mb-1">
                            <div className="fw-bold text-dark text-truncate pe-2">{chat.userName || chat.name}</div>
                            {unread > 0 && <span className="badge rounded-pill bg-danger px-2 py-1">{unread}</span>}
                          </div>
              ...
              <div className="flex-grow-1 d-flex flex-column" style={{ backgroundColor: '#fdfdfd' }}>
                {activeChat ? (
                  <>
                    <div className="bg-white p-4 border-bottom d-flex align-items-center gap-3 shadow-sm z-1">
                      <img src={activeChat.avatar || 'https://via.placeholder.com/40'} alt="avatar" className="rounded-circle border shadow-sm" style={{ width: '45px', height: '45px', objectFit: 'cover' }} />
                      <div className="fw-black fs-5 text-dark">{activeChat.userName || activeChat.name}</div>
                    </div>
                    ...
                    <div className="flex-grow-1 p-4 overflow-auto d-flex flex-column gap-3">
                      {activeChat.messages.map((msg, index) => (
                        <div key={index} className={`d-flex ${msg.sender === 'shop' ? 'justify-content-end' : 'justify-content-start'}`}>
                          <div className={`p-3 rounded-4 shadow-sm ${msg.sender === 'shop' ? 'bg-danger text-white' : 'bg-white text-dark border'}`} style={{ maxWidth: '80%', fontSize: '15px', lineHeight: '1.5' }}>
                            {msg.text}
                          </div>
                        </div>
                      ))}
                    ...
                    <form onSubmit={handleSendMessage} className="bg-white p-3 border-top d-flex gap-3 align-items-center shadow-sm z-1">
                      <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Nhập tin nhắn hỗ trợ..." className="form-control bg-light border-0 rounded-pill px-4 py-3" />
                      <button type="submit" className="btn btn-danger rounded-circle shadow-sm d-flex justify-content-center align-items-center flex-shrink-0" style={{ width: '50px', height: '50px' }}><FaPaperPlane size={18} /></button>
                    </form>
                  </>
                ) : (
                  <div className="w-100 h-100 d-flex flex-column justify-content-center align-items-center text-muted fs-5 opacity-50">
                    <FaComments size={60} className="mb-3" />
                    Vui lòng chọn khách hàng để bắt đầu chat.
                  </div>
                )}
              </div>
            </div>
          ...
          {activeTab === 'finance' && (
            <div>
              <div className="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 gap-3">
                <h1 className="fs-3 text-dark m-0 fw-black">Tài chính & Đối soát</h1>
                <button onClick={handleExportExcel} className="btn btn-outline-success fw-bold px-4 py-2 rounded-pill shadow-sm d-flex align-items-center justify-content-center gap-2">
                  <FaFileExcel /> Xuất Báo Cáo CSV
                </button>
              </div>

              <div className="row g-4">
                <div className="col-12 col-lg-5">
                  <div className="text-white p-5 rounded-4 shadow d-flex flex-column justify-content-between h-100 border-0" style={{ background: 'linear-gradient(135deg, #ff4b2b, #ff416c)' }}>
                    <div>
                      <div className="fs-5 opacity-75 d-flex align-items-center gap-2 mb-2 text-uppercase fw-bold" style={{ letterSpacing: '1px' }}><FaWallet /> Số dư khả dụng</div>
                      <div className="fw-black lh-1 mb-5" style={{ fontSize: 'clamp(2.5rem, 5vw, 3rem)' }}>{accountBalance.toLocaleString('vi-VN')} ₫</div>
                    </div>
                    <button onClick={handleWithdrawMoney} className="btn btn-light text-danger fw-bold rounded-pill px-5 py-3 align-self-start shadow-sm w-100 w-sm-auto text-uppercase">Rút Tiền Về Bank</button>
                  </div>
                </div>

                <div className="col-12 col-lg-7">
                  <div className="content-card p-5 h-100">
                    <h4 className="fs-4 text-dark fw-black d-flex align-items-center gap-3 border-bottom pb-4 mb-4"><FaFileInvoiceDollar className="text-primary"/> Bảng kê chi tiết {displayShopName}</h4>
                    <div className="d-flex flex-column gap-4 fs-5">
                      ...
                      <div className="d-flex justify-content-between text-secondary"><span>Chi phí Marketing:</span> <strong className="text-danger">-{marketingCost.toLocaleString('vi-VN')} ₫</strong></div>
                      <div className="d-flex justify-content-between text-secondary"><span>Tiền đã rút:</span> <strong className="text-primary">-{withdrawnAmount.toLocaleString('vi-VN')} ₫</strong></div>
                      <hr className="border-secondary border-dashed my-2" />
                      <div className="d-flex justify-content-between fs-4 fw-black text-dark"><span>Ví thực nhận:</span> <span className="text-success">{accountBalance.toLocaleString('vi-VN')} ₫</span></div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {/* TAB 8: CÔNG CỤ MARKETING DYNAMIC (Đồng bộ với Admin) */}
          {activeTab === 'marketing' && (
            <div>
              <h1 className="fs-3 text-dark mb-4 fw-black">Trung tâm Marketing</h1>
              <div className="row g-4">
                {marketingPackages.length === 0 ? (
                  <div className="col-12 text-center text-muted py-5">
                    Hệ thống chưa có gói dịch vụ nào.
                  </div>
                ) : (
                  marketingPackages.map((pkg, index) => {
                    const themeColor = getThemeForPackage(pkg.type);
                    return (
                      <div key={pkg.id || index} className="col-12 col-md-6 col-lg-4">
                        <div className={`content-card p-4 border-top border-${themeColor} border-4 h-100 d-flex flex-column`}>
                          <h3 className={`fs-5 text-${themeColor} fw-black mb-3`}>{pkg.name}</h3>
                          <p className="text-secondary lh-lg mb-4 flex-grow-1 small">{pkg.description}</p>
                          <div className="fw-black text-dark fs-5 mb-3">{Number(pkg.price).toLocaleString('vi-VN')} ₫ <span className="fs-6 text-muted fw-normal">/ chu kỳ</span></div>
                          <button onClick={() => handleBuyPackage(pkg.name, pkg.price)} className={`btn btn-${themeColor} w-100 py-2 fw-bold rounded-pill shadow-sm`}>
                            Đăng ký mua ngay
                          </button>
                        </div>
                      </div>
                ...
                <div className="col-12 col-md-6 col-lg-4">
                  <div className="content-card p-4 border border-dashed h-100 d-flex flex-column justify-content-center align-items-center opacity-50 bg-light">
                    <FaPlus size={30} className="text-muted mb-2" />
                    <span className="text-muted fw-bold">Đang cập nhật thêm</span>
                  </div>
                </div>
              </div>
            </div>
